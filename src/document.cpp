...
auto Document::position_from_byte(const std::size_t byte) const -> Position {
    const auto row{std::distance(this->line_indices_.begin(), std::ranges::upper_bound(this->line_indices_, byte) - 1)};
    const auto col{byte - this->line_indices_[row]};
...
void Document::update_line_indices_on_insert(const std::size_t pos, const std::string_view data) {
    const auto start{std::ranges::upper_bound(this->line_indices_, pos)};
