...
public:
    /// Sets the root node.
    void set_root(const std::shared_ptr<Viewport>& viewport);
    ...
    /// Splits the current Viewport into two.
    void split(bool vertical, float ratio, const std::shared_ptr<Viewport>& new_viewport);
