...
auto Workspace::find_viewport(const std::function<bool(const std::shared_ptr<Viewport>&)>& pred)
...
void Workspace::set_root(std::shared_ptr<Viewport> viewport) {
    this->switch_viewport([&] -> std::pair<bool, bool> {
        ...
        this->active_tree_viewport_ = this->root_->viewport_;
...
auto Workspace::render(Display& display, const sol::protected_function& resolve_face) -> bool {
...
void Workspace::exit_mini_buffer() {
    ...
    this->switch_viewport([&] -> std::pair<bool, bool> {
        ...
        if (auto prev = this->mini_buffer_.prev_viewport_.lock(); prev) {
            this->active_tree_viewport_ = prev;
        } else {
            this->active_tree_viewport_ = this->find_viewport([](const auto&) -> bool { return true; });
        }
...
void Workspace::split_root(bool vertical, float ratio, std::shared_ptr<Viewport> new_viewport) {
    ...
    if ((vertical && this->active_tree_viewport_->height_ < 8)
        || (!vertical && this->active_tree_viewport_->width_ < 25)) {
        return;
    }
    ...
    this->active_tree_viewport_ = new_leaf->viewport_;

    this->root_->resize(0, 0, this->width_, this->height_);
}

void Workspace::resize_split(float delta) {
    if (this->is_mini_buffer_ || !this->root_ || this->root_->viewport_ == this->active_tree_viewport_) { return; }

    // FIXME: replace _N with _ when upgrading to C++26.
    auto [parent, _1] = this->root_->find_parent(this->active_tree_viewport_);
...
auto Workspace::close_split() -> std::optional<std::shared_ptr<Viewport>> {
    ...
    const auto prev = this->active_tree_viewport_;
...
void Workspace::_split(bool vertical, float ratio, std::shared_ptr<Viewport> new_viewport) {
    if (!this->root_ || !this->active_tree_viewport_) { return; }
    ...
    // No Split exists yet.
    if (this->root_->viewport_ == this->active_tree_viewport_) {
        this->root_ = std::make_shared<Window>(this->root_, new_leaf, vertical);
        this->root_->ratio_ = ratio;
        this->active_tree_viewport_ = new_leaf->viewport_;
    ...
    auto [parent, child] = this->root_->find_parent(this->active_tree_viewport_);
...
void Workspace::_navigate_split(Direction direction) {
    ...
    if (!this->root_->get_path(this->active_tree_viewport_, path)) { return; }
    ...
    for (auto& [window, child]: std::ranges::reverse_view(path)) {
        ...
        if (can_move) {
            ...
            this->active_tree_viewport_ = sibling->edge_leaf(prefer_first);
            return;
        }
    }
}
...
auto Workspace::_close_split() -> std::shared_ptr<Viewport> {
    if (!this->root_ || !this->active_tree_viewport_) { return nullptr; }
    ...
    *parent = *new_node;
    ...
    if (new_node->viewport_) {
        this->active_tree_viewport_ = new_node->viewport_;
    } else {
        this->active_tree_viewport_ = new_node->find_viewport();
    }

    this->root_->resize(0, 0, this->width_, this->height_);

    return this->active_tree_viewport_;
}

void Workspace::switch_viewport(std::function<std::pair<bool, bool>()>&& f) {
    const auto prev = this->is_mini_buffer_ ? this->mini_buffer_.viewport_ : this->active_tree_viewport_;

    // Viewport switching.
    const auto [next_doc_loaded, prev_doc_unloaded] = f();

    const auto next = this->is_mini_buffer_ ? this->mini_buffer_.viewport_ : this->active_tree_viewport_;
    if (prev != next) {
