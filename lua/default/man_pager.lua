...
function ManPager.setup()
    ...
    Core.Modes.register_mode({
        ...
        mode_line_layout = {
            { callback = function(_) return { { text = "MAN" } } end },
    ...
    Core.Hooks.add("document::set-major-mode", {
            id = "man_pager.setup",
            priority = 50,
            metadata = { description = "Parses the man pager format." }
        },
        function(doc, mode)
            ...
            while idx <= #raw do
                ...
                if idx + len <= #raw and raw:sub(idx + len, idx + len) == '\x08' then
                    ...
                    if ch == '_' then
                        table.insert(styles, {
                            start = curr_byte,
                            stop = curr_byte + draw_ch_len,
                            face = Core.Face({ underline = true }),
                        })
                    elseif ch == draw_ch then
                        table.insert(styles, {
                            start = curr_byte,
                            stop = curr_byte + draw_ch_len,
                            face = Core.Face({ bold = true })
                        })
                    end
    ...
    Core.Hooks.add("document_view::created", {
            id = "man_pager.synchronize_new_document_view",
            priority = 50,
            metadata = { description = "Sets up new DocumentViews for a man pager Document." }
        },
        function(view)
