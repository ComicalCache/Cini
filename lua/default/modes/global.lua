...
function Global.init()
    ...
    Core.Keybinds.bind("global", "<C-g>", function()
        State.editor.viewport:toggle_gutter()
    end)
    ...
    Core.Keybinds.bind("global", "<", function()
        State.editor.viewport:move_cursor(function(cur, doc, _) cur:_jump_to_beginning_of_line(doc) end, 1)
    end)
    Core.Keybinds.bind("global", ">", function()
        State.editor.viewport:move_cursor(function(cur, doc, _) cur:_jump_to_end_of_line(doc) end, 1)
    end)
    Core.Keybinds.bind("global", "<S-g>", function()
        State.editor.viewport:move_cursor(function(cur, doc, _) cur:_jump_to_beginning_of_file(doc) end, 1)
    end)
    Core.Keybinds.bind("global", "g", function()
        State.editor.viewport:move_cursor(function(cur, doc, _) cur:_jump_to_end_of_file(doc) end, 1)
    end)
    Core.Keybinds.bind("global", "w", function()
        State.editor.viewport:move_cursor(Core.Cursor._next_word, 1)
    end)
    Core.Keybinds.bind("global", "<S-w>", function()
        State.editor.viewport:move_cursor(Core.Cursor._next_word_end, 1)
    end)
    Core.Keybinds.bind("global", "b", function()
        State.editor.viewport:move_cursor(Core.Cursor._prev_word, 1)
    end)
    Core.Keybinds.bind("global", "<S-b>", function()
        State.editor.viewport:move_cursor(Core.Cursor._prev_word_end, 1)
    end)
    Core.Keybinds.bind("global", "s", function()
        State.editor.viewport:move_cursor(Core.Cursor._next_whitespace, 1)
    end)
    Core.Keybinds.bind("global", "<S-s>", function()
        State.editor.viewport:move_cursor(Core.Cursor._prev_whitespace, 1)
    end)
    Core.Keybinds.bind("global", "}", function()
        State.editor.viewport:move_cursor(Core.Cursor._next_empty_line, 1)
    end)
    Core.Keybinds.bind("global", "{", function()
        State.editor.viewport:move_cursor(Core.Cursor._prev_empty_line, 1)
    end)
    Core.Keybinds.bind("global", ".", function()
        State.editor.viewport:move_cursor(function(cur, doc, _) cur:_jump_to_matching_opposite(doc) end, 1)
    end)

    Core.Keybinds.bind("global", "<S-h>", function()
        State.editor.viewport:scroll_left(1)
    end)
    Core.Keybinds.bind("global", "<S-j>", function()
        State.editor.viewport:scroll_down(1)
    end)
    Core.Keybinds.bind("global", "<S-k>", function()
        State.editor.viewport:scroll_up(1)
    end)
    Core.Keybinds.bind("global", "<S-l>", function()
        State.editor.viewport:scroll_right(1)
    end)

    Core.Keybinds.bind("global", "<C-w> <S-v>", function()
        State.editor:split_vertical(0.5)
    end)
    Core.Keybinds.bind("global", "<C-w> <S-h>", function()
        State.editor:split_horizontal(0.5)
    end)
    Core.Keybinds.bind("global", "<C-w> +", function()
        State.editor:resize_split(0.05)
    end)
    Core.Keybinds.bind("global", "<C-w> -", function()
        State.editor:resize_split(-0.05)
    end)
    ...
    Core.Keybinds.bind("global", "<C-w> h", function()
        State.editor:navigate_splits(Core.Direction.Left)
    end)
    Core.Keybinds.bind("global", "<C-w> j", function()
        State.editor:navigate_splits(Core.Direction.Down)
    end)
    Core.Keybinds.bind("global", "<C-w> k", function()
        State.editor:navigate_splits(Core.Direction.Up)
    end)
    Core.Keybinds.bind("global", "<C-w> l", function()
        State.editor:navigate_splits(Core.Direction.Right)
    end)

    Core.Keybinds.bind("global", "+", function()
        State.editor.viewport:toggle_mode_line()
    end)

    Core.Keybinds.bind("global", "<Space>", function()
        State.editor:enter_mini_buffer()
        Core.Modes.set_major_mode(State.editor.mini_buffer.doc, "mini_buffer")
    end)

    Core.Keybinds.bind("global", "i", function()
        Core.Modes.add_minor_mode(State.editor.viewport.doc, "insert")
    end)
    Core.Keybinds.bind("global", "<C-s>", function()
        State.editor.viewport.doc:save(nil)
