...
function Insert.init()
    Core.Keybinds.bind("insert", "<Esc>", function()
        Core.Modes.remove_minor_mode(State.editor.viewport.doc, "insert")
    end)

    Core.Keybinds.bind("insert", "<Left>", function()
        State.editor.viewport:move_cursor(Core.Cursor.left, 1)
    end)
    ...
    Core.Keybinds.bind("insert", "<Right>", function()
        State.editor.viewport:move_cursor(Core.Cursor.right, 1)
    end)
    ...
    Core.Keybinds.bind("insert", "<Enter>", function()
        ...
        doc:insert(doc.point, "\n")
        State.editor.viewport:move_cursor(Core.Cursor.down, 1)
        State.editor.viewport:move_cursor(function(cur, d, _) cur:_jump_to_beginning_of_line(d) end, 1)
    end)
