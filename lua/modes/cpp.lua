...
function M.setup()
    ...
    local keyword_face = Core.Face({ fg = Core.Rgb(198, 120, 221) })
    local type_face = Core.Face({ fg = Core.Rgb(229, 192, 123) })
    local string_face = Core.Face({ fg = Core.Rgb(152, 195, 121) })
    local number_face = Core.Face({ fg = Core.Rgb(209, 154, 102) })
    local comment_face = Core.Face({ fg = Core.Rgb(92, 99, 112) })
    local preproc_face = Core.Face({ fg = Core.Rgb(224, 108, 117) })

    -- Register Faces
    cpp:set_face("cpp:keyword", keyword_face)
    cpp:set_face("cpp:type", type_face)
    cpp:set_face("cpp:string", string_face)
    cpp:set_face("cpp:number", number_face)
    cpp:set_face("cpp:comment", comment_face)
    cpp:set_face("cpp:preproc", preproc_face)
